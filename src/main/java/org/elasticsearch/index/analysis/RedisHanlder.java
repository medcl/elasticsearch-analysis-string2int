...
public class RedisHanlder {
    ...
    private static Map<String,RedisHanlder> instance=new HashMap<String,RedisHanlder>();
    ...
    private long get_from_cache(String redis_key,String item){
      if(!local_cache.containsKey(redis_key)){return  -1;}
      if(!local_cache.get(redis_key).containsKey(item)){return -1;}
    ...
    private void set_local_cache(String redis_key,String item,long  value){
        if(value==-1){return;}
        if(!local_cache.containsKey(redis_key)){local_cache.put(redis_key,new HashMap<String, Long>());}
        if(!local_cache.get(redis_key).containsKey(item)){
            local_cache.get(redis_key).put(item, value);
        }
    }
